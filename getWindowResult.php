<?php
	...
	*/
	...
	//sleep(5);
		...
		public function __construct($id, $time, $title, $duration) {
		  $this->id = $id;
		  $this->time = $time;
		  $this->title = $title;
	...
	function getDetails($from,$to,$filter,$myhost,$nbrecs) {
		...
		$query = "
		SELECT fgw_time,fgw_title,fgw_duration,fgw_cpu
		FROM fgw
		WHERE
		...
		if ($result->num_rows > 0) {
			...
			while($rs = $result->fetch_assoc()) {
				...
				$myfgw = new Fgw("",$time,utf8_encode($title),number_format(strval($duration),0));
				$fgws[] = $myfgw;
			}
		} else {
